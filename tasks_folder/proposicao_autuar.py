...
@zope_task(bind=True, max_retries=5, default_retry_delay=10)
def proposicao_autuar_task(self, site, cod_proposicao, portal_url):
    ...
    if self.request.retries > 0:
        logging.warning(f"[proposicao_autuar_task] Retry #{self.request.retries} | Tarefa ID={task_id} | Fila: {queue_name} | cod_proposicao={cod_proposicao}")
    else:
        logging.info(f"[proposicao_autuar_task] Iniciando tarefa | ID={task_id} | Fila: {queue_name} | cod_proposicao={cod_proposicao}")
    ...
    try:
        ...
        try:
            ...
            logging.info("[proposicao_autuar_task] PDF reparado com pikepdf.")
            stream = repaired_stream
        except Exception as e:
            logging.warning(f"[proposicao_autuar_task] Falha ao reparar com pikepdf: {e}")
            original_stream.seek(0)
        ...
        logging.info(f"[proposicao_autuar_task] Finalizado com sucesso | cod_proposicao={cod_proposicao} | gerado: {nom_pdf_saida}")
        return nom_pdf_saida

    except Exception as e:
        logging.exception(f"[proposicao_autuar_task] Erro durante execução (irá tentar retry): {e}")
