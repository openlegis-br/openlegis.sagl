...
def criar_documento(tip_doc, txt_ementa, nome_autor, cod_materia_respondida, cod_protocolo, num_protocolo, cod_usuario):
    ...
    if context.dbcon_logs:
       context.zsql.logs_registrar_zsql(usuario = REQUEST['AUTHENTICATED_USER'].getUserName(), data = DateTime(datefmt='international').strftime('%Y-%m-%d %H:%M:%S'), modulo = 'documento_acessorio_materia', metodo = 'resposta_executivo_pysc', cod_registro = cod_documento, IP = context.pysc.get_ip())
...
def tramitar_materia(cod_documento, cod_materia, cod_protocolo, num_protocolo, cod_usuario):
    ...
    txt_tramitacao = '<p>Resposta eletrônica recebida em ' + hr_tramitacao + ' sob protocolo nº ' + str(hdn_num_protocolo) + '/' + DateTime(datefmt='international').strftime("%Y") +'</p>'
    context.zsql.tramitacao_incluir_zsql(cod_materia=cod_materia, dat_tramitacao=DateTime(datefmt='international').strftime('%Y-%m-%d %H:%M:%S'), cod_unid_tram_local=4, cod_usuario_local=cod_usuario, cod_unid_tram_dest=19, dat_encaminha=DateTime(datefmt='international').strftime('%Y-%m-%d %H:%M:%S'), cod_status=1056, ind_urgencia=0, txt_tramitacao = txt_tramitacao, ind_ult_tramitacao=1)
    ...
    for tramitacao in context.zsql.tramitacao_obter_zsql(cod_materia=cod_materia, ind_ult_tramitacao=1, ind_excluido=0):
        ...
        if context.dbcon_logs:
           context.zsql.logs_registrar_zsql(usuario = REQUEST['AUTHENTICATED_USER'].getUserName(), data = DateTime(datefmt='international').strftime('%Y-%m-%d %H:%M:%S'), modulo = 'tramitacao_materia', metodo = 'resposta_executivo_pysc', cod_registro = tramitacao.cod_tramitacao, IP = context.pysc.get_ip())
...
def criar_reposta(cod_protocolo, num_protocolo):
    ...
    dic_resposta['data_protocolo'] = DateTime(datefmt='international').strftime('%d/%m/%Y %H:%M:%S')
