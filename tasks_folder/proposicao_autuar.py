...
def proposicao_autuar_task(portal, cod_proposicao, portal_url):
    ...
    try:
        # Read PDF data once
        pdf_data = BytesIO(bytes(arq.data))
        ...
        if fields:
            ...
            if signature_field_values:
                # Use the pdf_data again here
                pdf_data.seek(0)  # Reset the stream position
                signers = get_signatures(pdf_data)
        ...
        if qtde_assinaturas > 2:
            outros = " e outros"
        for proposicao in skins.zsql.proposicao_obter_zsql(
            ...
            if proposicao.ind_mat_ou_doc == "M":
                for materia in skins.zsql.materia_obter_zsql(
                    cod_materia=proposicao.cod_mat_ou_doc
                ):
                    if (
                        materia.num_protocolo != None
                        and materia.num_protocolo != ""
                    ):
                        for protocolo in skins.zsql.protocolo_obter_zsql(
                            num_protocolo=materia.num_protocolo,
                            ano_protocolo=materia.ano_ident_basica,
                        ):
                            info_protocolo = (
                                " - Prot. "
                ...
                proposicao.ind_mat_ou_doc == "D"
                and (
                    proposicao.des_tipo_proposicao != "Emenda"
                    and proposicao.des_tipo_proposicao != "Mensagem Aditiva"
                    and proposicao.des_tipo_proposicao != "Substitutivo"
                    and proposicao.des_tipo_proposicao != "Parecer"
                    and proposicao.des_tipo_proposicao != "Parecer de Comissão"
                )
            ):
                for documento in skins.zsql.documento_acessorio_obter_zsql(
                    cod_documento=proposicao.cod_mat_ou_doc
                ):
                    for materia in skins.zsql.materia_obter_zsql(
                        cod_materia=documento.cod_materia
                    ):
                        materia = (
                            str(materia.sgl_tipo_materia)
                            + " nº "
                            + str(materia.num_ident_basica)
                            + "/"
                            + str(materia.ano_ident_basica)
                ...
                and (
                    proposicao.des_tipo_proposicao == "Emenda"
                    or proposicao.des_tipo_proposicao == "Mensagem Aditiva"
                )
            ):
                for emenda in skins.zsql.emenda_obter_zsql(
                    cod_emenda=proposicao.cod_emenda
                ):
                    for materia in skins.zsql.materia_obter_zsql(
                        cod_materia=emenda.cod_materia
                    ):
                        materia = (
                            str(materia.sgl_tipo_materia)
                            + " nº "
                ...
                texto = (
                    "Emenda "
                    + str(emenda.des_tipo_emenda)
                    + " nº "
                    + str(emenda.num_emenda)
                    + " ao "
                    + str(materia)
                )
                storage_path = portal.sapl_documentos.emenda
                nom_pdf_saida = str(emenda.cod_emenda) + "_emenda.pdf"
            elif proposicao.ind_mat_ou_doc == "D" and (
                proposicao.des_tipo_proposicao == "Substitutivo"
            ):
                for substitutivo in skins.zsql.substitutivo_obter_zsql(
                    cod_substitutivo=proposicao.cod_substitutivo
                ):
                    for materia in skins.zsql.materia_obter_zsql(
                        cod_materia=substitutivo.cod_materia
                    ):
                        materia = (
                            str(materia.sgl_tipo_materia)
                            + " nº "
                    ...
                    texto = (
                        "Substitutivo nº "
                        + str(substitutivo.num_substitutivo)
                        + " ao "
                        + str(materia)
                    )
                    storage_path = portal.sapl_documentos.substitutivo
                    nom_pdf_saida = (
                        str(substitutivo.cod_substitutivo) + "_substitutivo.pdf"
                    )
            elif (
                proposicao.ind_mat_ou_doc == "D"
                and (
                    proposicao.des_tipo_proposicao == "Parecer"
                    or proposicao.des_tipo_proposicao == "Parecer de Comissão"
                )
            ):
                for relatoria in skins.zsql.relatoria_obter_zsql(
                    cod_relatoria=proposicao.cod_parecer, ind_excluido=0
                ):
                    for comissao in skins.zsql.comissao_obter_zsql(
                        cod_comissao=relatoria.cod_comissao
                    ):
                        sgl_comissao = comissao.sgl_comissao
                    for materia in skins.zsql.materia_obter_zsql(
                        cod_materia=relatoria.cod_materia
                    ):
                        materia = (
                            str(materia.sgl_tipo_materia)
                            + " nº "
                    ...
                    texto = (
                        ...
                        + sgl_comissao
                        + " nº "
                        + str(relatoria.num_parecer)
                        + "/"
                        + str(relatoria.ano_parecer)
                        + " ao "
                        + str(materia)
                    )
            ...
            for validacao in skins.zsql.assinatura_documento_obter_zsql(
                codigo=cod_proposicao, tipo_doc="proposicao", ind_assinado=1
            ):
                ...
                stream = make_qrcode(
                    ...
                    + str(validacao.cod_assinatura_doc)
                )
                mensagem1 = (
                    f"Esta é uma cópia do original assinado digitalmente por {nom_autor}{outros}"
                )
                mensagem2 = (
                    f"Para validar visite {portal_url}/conferir_assinatura e informe o código {cod_validacao_doc}"
                )
            # Open PDF with pymupdf after all processing
            pdf_data.seek(0)  # Reset stream again
            existing_pdf = pymupdf.open(stream=pdf_data)
            ...
            for page_index, i in enumerate(range(len(existing_pdf))):
                ...
                existing_pdf[page_index].insert_text(
                    (x, y), text3, fontsize=8, rotate=90
                )
                # logo icp
            ...
            existing_pdf[0].insert_textbox(
                rect, str(texto).upper(), fontname="tibo", fontsize=13, align=pymupdf.TEXT_ALIGN_CENTER
            )
            ...
            return nom_pdf_proposicao
    except Exception as e:
        logging.error(f"Erro ao autuar proposição: {e}")
        return None
