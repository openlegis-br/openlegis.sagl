...
class otimizarPDF(grok.View):
    ...
    def check_signatures(self, fileStream):
        with open("/home/luciano/14956_signed.pdf", "rb") as f:
             doc = BytesIO(f.read())
        ...
        if fields != None:
            ...
            if signature_field_values is not None:
               return get_signatures(fileStream)
            else:
               # tratar documento e colocar metadados com pymupdf
    ...
    def parse_signatures(self, raw_signature_data):
        info = cms.ContentInfo.load(signature_data)
    ...
    def render(self, fileStream):
        return self.check_signatures(fileStream)
