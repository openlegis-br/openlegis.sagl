...
class SessoesPlenarias(grok.View):
	...
	for item in self.context.zsql.sessao_plenaria_obter_zsql(cod_sessao_plen=id, tip_sessao=tipo, ano_sessao=ano, ind_excluido=0):
	    dic = {}
	    dic['@id'] = portal_url + '/@@sessoes_plenarias?id=' + str(item.cod_sessao_plen)
	    ...
	    dic['description'] = str(item.num_sessao_plen) + 'ª ' + 'Reunião ' +  dic['tipo'] + ' da ' + str(item.num_sessao_leg) + 'ª Sessão Legislativa da ' + str(item.num_legislatura) + 'ª Legislatura'
			...
			if ordem.cod_parecer != None:
			   ...
			   if hasattr(self.context.sapl_documentos.materia, arquivo):
			      ...
			      lst_arquivo.append(dic_arquivo)
		           ...
		           dic_votacao['remoteUrl'] = portal_url + '/consultas/materia/materia_mostrar_proc?cod_materia=' + ordem.cod_materia
