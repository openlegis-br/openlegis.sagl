...
class MesasDiretoras(grok.View):
	...
	for item in self.context.zsql.periodo_comp_mesa_obter_zsql(ind_excluido=0):
	    ...
	    dic['start'] = DateTime(item.dat_inicio_periodo).strftime("%Y-%m-%d")
	    dic['end'] = DateTime(item.dat_fim_periodo).strftime("%Y-%m-%d")
	    dic['title'] = 'Período de composição'
	    dic['legislatura'] = str(item.num_legislatura)
            dic['description'] = DateTime(item.dat_inicio_periodo).strftime("%d/%m/%Y") + ' a ' + DateTime(item.dat_fim_periodo).strftime("%d/%m/%Y")
            if (DateTime().strftime("%Y-%m-%d") > DateTime(item.dat_inicio_periodo).strftime("%Y-%m-%d") and DateTime().strftime("%Y-%m-%d") < DateTime(item.dat_fim_periodo).strftime("%Y-%m-%d")):
               dic['atual'] = True
            else:
               dic['atual'] = False
            lista.append(dic)
