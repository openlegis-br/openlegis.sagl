...
class Materia(grok.View):
    ...
    def render(self, id):
	    ...
	    dic_materia['data_apresentacao'] = DateTime(item.dat_apresentacao, datefmt='international').strftime("%Y-%m-%d")
	    lst_arquivo = []
	    dic_arquivo = {}	    
		...
		dic_materia["regime_tramitacao_id"] = str(regime.cod_regime_tramitacao)
	        ...
	        dic_anexada['@type'] = 'Matéria Legislativa'
                ...
                dic_anexada['data_anexacao'] = DateTime(anexada.dat_anexacao, datefmt='international').strftime("%Y-%m-%d")
	    ...
	    dic_materia["anexada"] = lst_anexada
	        ...
	        dic_documento['@type'] = 'Documento Acessório'
                ...
                dic_documento['data'] = DateTime(documento.dat_documento, datefmt='international').strftime("%Y-%m-%d")
	        lst_arquivo = []
	        dic_arquivo = {}	    
	    ...
	    dic_materia["emenda"] = lst_emenda
	        ...
	        dic_substitutivo["autoria"] = lista_autor
                dic_substitutivo['data'] = DateTime(substitutivo.dat_apresentacao, datefmt='international').strftime("%Y-%m-%d")
	        lst_arquivo = []
		...
		if parecer.tip_conclusao == 'F':
		   ...
		   dic_parecer['description'] = 'Relatoria de '+ relator.nom_parlamentar + ', com voto CONTRÁRIO'
                dic_parecer['data'] = DateTime(parecer.dat_destit_relator, datefmt='international').strftime("%Y-%m-%d")
                lista_autor = []
                dic_autor = {}
		...
		dic_votacao["data"] = DateTime(sessao_plenaria.dat_inicio_sessao, datefmt='international').strftime("%Y-%m-%d")
		dic_votacao["turno"] = ''
