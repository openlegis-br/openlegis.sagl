...
for materia in context.zsql.materia_obter_zsql(cod_materia=cod_materia):
  ...
  if texto_original:
    download_name = str(materia.sgl_tipo_materia)+"-"+str(materia.num_ident_basica)+"-"+str(materia.ano_ident_basica)+".pdf"
    id_documento = "%s"%cod_materia+'_texto_integral.pdf'

  if redacao_final:
    download_name = str(materia.sgl_tipo_materia)+"-"+str(materia.num_ident_basica)+"-"+str(materia.ano_ident_basica)+"_redacao_final.pdf"
