...
def pauta(pauta_dic):
    ...
    if pauta_dic["nom_sessao"] == 'AUDIÊNCIA PÚBLICA':
       ...
       tmp+='\t\t</para>\n'
       tmp+='\t\t<para style="P0">EM ' + str(pauta_dic["dia_sessao"]) + ' ÀS ' + str(pauta_dic["hr_inicio_sessao"]) + '</para>\n'
       ...
       if pauta_dic["num_periodo"] != '':
          ...
          tmp+='\t\t<para style="P0">'+ str(pauta_dic["num_sessao_plen"]) +'ª ' + str(context.sapl_documentos.props_sagl.reuniao_sessao).upper() + ' ' + pauta_dic["nom_sessao"] + ' - ' + pauta_dic["dat_inicio_sessao"] + ' ÀS ' + str(pauta_dic["hr_inicio_sessao"]) + '</para>\n'
       tmp+='\t\t<para style="P2" spaceAfter="4">\n'
       tmp+='\t\t\t<font color="white"> </font>\n'
       tmp+='\t\t</para>\n'
    ...
    if pauta_dic["lst_expedientes"] != [] or pauta_dic["lst_materia_apresentada"] or pauta_dic["lst_materia_apresentada"] != [] or pauta_dic["lst_requerimentos_vereadores"] != [] or pauta_dic["lst_mocoes_vereadores"] != [] or pauta_dic["lst_pedidos_vereadores"] != []:
       tmp+='\t\t<para style="P0" spaceBefore="10" spaceAfter="10"><b><u>EXPEDIENTE</u></b></para>\n\n'
    ...
    if pauta_dic["lst_pedidos_vereadores"] != []:
        tmp+='\t\t<condPageBreak height="20mm"/>\n'
        tmp+='\t\t<para style="P1" spaceBefore="20"><b>DISCUSSÃO E VOTAÇÃO DE PEDIDOS DE INFORMAÇÃO</b></para>\n'
    for dic in pauta_dic["lst_pedidos_vereadores"]:
        tmp+='\t\t<para style="P2" spaceBefore="10"><b><u>' + dic['vereador'] + '</u></b>:</para>\n'
        for item in dic['materias']:
            tmp+='\t\t<para style="P2" spaceBefore="5"><font color="#126e90"><b>' + item['id_materia'] + '</b></font> - ' + escape(item['txt_ementa']) + '</para>\n'
    ...
    for dic in pauta_dic["lst_urgencia"]:
        tmp+='\t\t<para style="P2" spaceBefore="15" spaceAfter="3"><b>' + str(dic['num_ordem']) +') <font color="#126e90">' + dic['id_materia'] + '</font> - ' + dic['nom_autor'] + '</b> - ' + escape(dic['txt_ementa']) + '</para>\n'
        ...
        if dic['substitutivo']!= 0 and dic['substitutivo']!= '':
            ...
            for substitutivo in dic['substitutivos']:
                tmp+='\t\t<para style="P3" spaceAfter="3"><font color="#126e90">' + substitutivo["id_substitutivo"] + '</font> - ' + substitutivo["autoria"] + ' - ' + escape(substitutivo['txt_ementa']) + '</para>\n'

        if dic['emenda']!= 0 and dic['emenda']!= '':
            tmp+='\t\t<para style="P3" spaceAfter="3"><b>Emendas:</b></para>\n\n'
            for emenda in dic['emendas']:
                tmp+='\t\t<para style="P3" spaceAfter="3"><font color="#126e90">' + emenda["id_emenda"] + '</font> - ' + emenda["autoria"] + ' - ' + escape(emenda['txt_ementa']) + '</para>\n'

        if dic['parecer']!= 0 and dic['parecer']!= '':
            tmp+='\t\t<para style="P3" spaceAfter="3"><b>Pareceres:</b></para>\n\n'
            for item in dic['pareceres']:
                tmp+='\t\t<para style="P3" spaceAfter="2"><font color="#126e90">' + item["id_parecer"] + '</font>, com <b>' + item["conclusao"] + '</b> ' + item["relatoria"] + ', <b>' + item["resultado"] + '</b> na ' + item["comissao"] + '</para>\n'
    ...
    for dic in pauta_dic["lst_pauta"]:
        tmp+='\t\t<para style="P2" spaceBefore="15" spaceAfter="3"><b>' + str(dic['num_ordem']) +') <font color="#126e90"><u>' + dic['id_materia'] + '</u></font> - ' + dic['nom_autor'] + '</b> - ' + escape(dic['txt_ementa']) + '</para>\n'
        tmp+='\t\t<para style="P3" spaceAfter="3"><b>Turno</b>: '+ dic["des_turno"] +' | <b>Quorum</b>: '+ dic['des_quorum']+' | <b>Tipo de Votação</b>: '+ dic['tip_votacao'] + '' + '</para>\n'
...
def principal(dic_cabecalho, dic_rodape, imagem, pauta_dic):
    ...
    tmp+='\t<template pageSize="(21cm, 29.7cm)" title="Pauta da Sessão" author="SAGL" allowSplitting="20">\n'
