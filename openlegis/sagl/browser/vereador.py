...
class VereadoresAtuais(grok.View):
	...
	dic_resultado = {}
    	dic_resultado['@id'] = portal_url + '/@@vereadores_atuais'
       	dic_resultado['@type'] = 'Vereadores'
       	dic_resultado['description'] = 'Lista de Vereadores em Exercício'
        dic_resultado['items'] = lista_exercicio
...
class Vereador(grok.View):
    ...
    def render(self, id):
		...
		dic_partido = {}
		dic_partido['data_filiacao'] = DateTime(filiacao.dat_filiacao, datefmt='international').strftime("%Y-%m-%d")
		...
		if mandato.ind_titular == 1:
                ...
                dic_mandato['fim'] = DateTime(mandato.dat_fim_mandato, datefmt='international').strftime("%Y-%m-%d")
