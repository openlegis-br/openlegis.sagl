...
function createEditor() {
    ...
    function guessDocUrl() {
        ...
        if (pos !== -1) {
            ...
            docUrl = "<dtml-var portal_url>/sagl/sapl_documentos/sapl_documentos/proposicao/60.odt";
        }
        return docUrl || null;
    }
    ...
    function fileSelectHandler(evt) {
        ...
        if (files && files.length === 1) {
            if (!editor.isDocumentModified() ||
                window.confirm("Existem alterações não salvas neste documento. Deseja descartá-las?")) {
